...
def test(test_filename, train_dataset_dir, test_dataset_dir, model_dir, model_types, voting_type='hard_voting', select_fold=None):
	'''
	...
	if select_fold == None:
		...
		ensemble_predictions = [max(set(i), key = list(i).count) for i in all_predictions]
