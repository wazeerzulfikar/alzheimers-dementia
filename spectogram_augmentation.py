...
def augment_freq_time_mask(spectrogram,
                           ...
                           time_mask_num=3):
    num_channels = tf.shape(spectrogram)[3]
    time_max = tf.shape(spectrogram)[2]
    ...
    for _ in range(frequency_mask_num):
        ...
        value_ones_freq_prev = tf.ones(shape=[batch_size, f0, time_max, num_channels])
        value_zeros_freq = tf.zeros(shape=[batch_size, f, time_max, num_channels])
        value_ones_freq_next = tf.ones(shape=[batch_size, freq_max-(f0+f), time_max, num_channels])
    ...
    for _ in range(time_mask_num):
        ...
        value_zeros_time_prev = tf.ones(shape=[batch_size, freq_max, t0, num_channels])
        value_zeros_time = tf.zeros(shape=[batch_size, freq_max, t, num_channels])
        value_zeros_time_next = tf.ones(shape=[batch_size, freq_max, time_max-(t0+t), num_channels])
...
def augment_pitch_and_tempo(spectrogram,
    ...
    spectrogram_aug = tf.image.resize(spectrogram, [new_freq_size, new_time_size])
    # spectrogram_aug = tf.image.resize_with_crop_or_pad(spectrogram_aug,
    #                                                                target_height=original_shape[1], target_width=original_shape[2])
