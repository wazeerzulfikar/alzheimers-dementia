...
dataset_dir = ''
dataset_dir = '../ADReSS-IS2020-data/train/cqt'
cc_files = sorted(glob.glob(os.path.join(dataset_dir, 'cc/*.npy')))
X_cc = [np.load(f) for f in cc_files]
...
X_cd = [np.load(f) for f in cd_files]
...
print(np.mean(lengths))
print(np.std(lengths))
print(np.array(X_cd).shape)

print(np.max(lengths))
print(np.min(lengths))

X = np.array(X_cc+X_cd)
...
def create_model(_type_ = 'convolutional'):
	...
	if _type_=='fully_convolutional':
		...
		model.add(layers.Input(inp_shape, ragged=True))
